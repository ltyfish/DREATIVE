...
- `approval`: contract-only hash, revision, source baseline and honest
  `human` or `pre-authorized-dogfood` origin.
...
dreative resume
dreative verify --browser-command "npm run dev"
dreative critic-run --command "<separate critic process>"
dreative audit
...
plan fields, reapproval under the correct mode, a new trusted browser run and a
new sealed critic run; old self-authored evidence is intentionally not
grandfathered.
