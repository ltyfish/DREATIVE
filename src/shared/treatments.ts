...
export function renderTreatmentSummary(selected: SpecialistSkill[], explicitAll = false): string {
  const lines = selected.map((name) => {
    ...
    return [
      ...
      `  Proposed role: ${item.defaultRole}. Acceptance example: ${item.acceptanceExample}`,
    ].join("\n");
  });
...
export function renderProposedTreatmentAllocation(selected: SpecialistSkill[], routes: string[] = []): string {
  const route = routes[0] ?? "target-route";
  const location = (name: SpecialistSkill) =>
    name === "ux" || name === "mobile" || name === "refined" ? `${route} — full route`
      : name === "immersive" || name === "cinematic" ? `${route} — hero → downstream chapters → resolution`
        : name === "experimental" ? `${route} — two or three selected peaks`
          : `${route} — hero plus at least one downstream chapter`;
  return [
    "Proposed treatment allocation (edit before approval):",
    ...selected.map((name) => {
      const item = treatment(name);
      const routeRole = item.defaultRole === "foundation" ? "foundation" : name === "immersive" || name === "cinematic" ? "connective tissue" : "peak";
      return `- ${name}: ${item.defaultRole} at ${location(name)}; ${routeRole}; accept when ${item.acceptanceExample.toLowerCase()}`;
    }),
  ].join("\n");
}
