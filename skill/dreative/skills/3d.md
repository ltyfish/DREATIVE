...
`motion` when scroll or time drives the scene. Deliverables: a named spatial
purpose, staged canvas/prop, asset and runtime budgets, semantic/poster fallback,
and measured runtime evidence. Do not load for ambient decoration. Done means the
subject, context-loss/mobile/reduced-motion fallbacks, and frame behavior pass.
...
**Form default, from shipped-run evidence: the rung-2 cutout billboard.**
Across real builds, the prop that consistently reads premium is the
transparent-background photoreal cutout sold with scene means (parallax,
damped rotation, contact shadow, rim light, multi-angle pseudo-orbit) —
because it IS a photo. Coded/shader hero geometry consistently reads as a
tech demo, however carefully lit. So unless a real GLB exists, the required
prop defaults to rung 2; reaching for a coded form instead of generating a
cutout is the failure, not a stylistic option.
